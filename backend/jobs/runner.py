...
async def _execute_pipeline(request_data: Mapping[str, Any]) -> str:
    ...
    scene_data, scene_json_text = unityconnector._load_scene_json(scene_json_path)
    ...
    if manifest_path.exists():
        manifest_data, views_manifest_text = unityconnector._load_views_manifest(manifest_path)
    else:
    ...
    if requested_names and manifest_data:
        selected_manifest_objects, missing_names = unityconnector._select_manifest_objects(
    ...
    object_selections = []
    if manifest_data:
        object_selections = unityconnector._collect_images_for_objects(scene_dir, selected_manifest_objects)

    total_images = sum(len(selection.images) for selection in object_selections)
    print(f"Images ready to send: {total_images}")

    include_images = unityconnector.SEND_IMAGES_TO_AGENT and start_pipeline
    ...
    _, fs_dir = unityconnector._output_dirs(group)
    batches = (
        unityconnector._chunk_objects(object_selections, unityconnector.MAX_OBJECTS_PER_RUN)
    ...
    for index, batch in enumerate(batches, start=1):
        ...
        content = unityconnector._build_input_items(
            task_text=task_text,
            bundle=input_bundle,
        ...
        if len(batches) > 1:
            batch_label = "_".join(unityconnector._safe_dir_name(selection.object_name) for selection in batch)
            if not batch_label:
