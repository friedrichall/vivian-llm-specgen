...
# Run backend (primary entry point)
fastapi dev backend/main.py
# Alternatives: python -m backend  OR  uvicorn backend.main:app --reload
...
python -m pytest tests/test_backend_api.py::test_start_job_rejects_legacy_redundant_fields

# Regenerate JSON schemas and docs (run in order after changing models in model/)
python scripts/export_schema.py
python scripts/build_docs.py

# Regenerate TypeScript/client DTOs
python scripts/generate_dtos.py
```

Required env var: `OPENAI_API_KEY`. Copy `.env.muster` → `.env` and fill values.
Optional: `LANGSMITH_TRACING=true` + `LANGSMITH_API_KEY` for LangSmith tracing.

## Architecture

### What this project does

Generates **FunctionalSpecification** JSON files for the **Vivian** 3D-prototype framework from Unity scene data, using a multi-agent LLM pipeline. A FunctionalSpec consists of five files: `InteractionElements.json`, `VisualizationElements.json`, `VisualizationArrays.json` (always `{"Elements":[]}`), `States.json`, `Transitions.json`.
...
`agents_vivian.py` is **legacy dead code**. It exists only as a standalone CLI demo script (`main.py → agents_vivian()`). Nothing in the API/job path imports it.

### PipelineOrchestrator (`vivian_pipeline/pipeline_orchestrator.py`)

The core class. It runs a deterministic attempt-loop (default 3 attempts):

1. **Scene analysis** — `scene_analysis_tool` runs the scene-analysis agent against scene JSON + images, returns `SceneUnderstanding`.
2. **Scene review** — publishes the `SceneUnderstanding` via `publish_scene_review` callback; the API stores it in `JobManager`. The loop awaits a user decision via `await_scene_decision` callback (polling an asyncio queue fed by `POST /v1/jobs/{job_id}/scene-review`). On feedback, `apply_scene_feedback()` mutates the scene object and the loop repeats.
3. **Spec generation** — four agents run sequentially, each receiving the confirmed scene + previously generated outputs as context:
   - `interaction_elements_agent` → `InteractionElementsFile`
   - `visualization_elements_agent` → `VisualizationElementsFile`
   - `states_agent` → `StatesFile`
   - `transitions_agent` → `TransitionsFile`
4. **Validation** — Unity validator runs on the draft `FunctionalSpecification/` directory.
5. **Retry** — on validator failure, `rerun_policy.py` maps errors to "dirty steps" and only those steps are re-run. Step dependencies expand downstream (e.g. dirty `states` → also dirty `transitions`).
6. **Publish** — on success, final files written to `final_output_dir`.

Scene confirmation is cached after the first attempt; subsequent attempts reuse `self.scene_confirmed`.

### Agent setup (`vivian_pipeline/agents_setup.py`)

Module-level singletons for the four spec agents + `scene_analysis_agent`. All use `BASE_MODEL`. The orchestrator imports them directly — no factory functions.

### Agent instructions

`constants/agent_instructions.py` loads instruction text from `docs_vivian/*DocuLLMFriendly` files. **`docs_vivian/` is the source of truth** for domain rules. Update docs there first, then re-test agents.

### Two Pydantic model layers

| Layer | Location | Used by |
|---|---|---|
| Per-agent output models | `vivian_pipeline/models_funcspec/` | `PipelineOrchestrator`, agents |
| Legacy full-spec model | `model/output_type_FuncSpec.py` + siblings | `agents_vivian.py` (legacy) only |

The orchestrator only uses `models_funcspec/`. Cross-reference validation (e.g. states referencing known element names) happens inside `PipelineOrchestrator` after each agent run.

### Backend job lifecycle (`backend/jobs/`)

- `JobManager` (singleton in `router.py`) holds one active job at a time. It owns the scene-review asyncio queues.
- `runner.py` runs jobs inside `async with manager.lock`, writes stdout to `job.log_path` via `capture_job_output()`.
- Phases emitted by `PipelineOrchestrator` via the `on_phase_change` callback update `job.phase` in memory and persist to `meta.json`.
- Log output for `GET /v1/jobs/{job_id}/logs` is chunk-read from `run.log` using byte offsets.

### Artifact layout

```
logs/
  orchestrator/runs/<YYYYMMDD-HHMMSS-NNN>/   # NNN = zero-padded daily counter (resets each day)
    run_meta.json
    registry_log.jsonl          # every registry mutation, append-only
    attempts/<n>/
  ...
  backend/jobs/<YYYYMMDD-HHMMSS-jobId>/
    ...
    meta.json                   # job snapshot
```

Attempt directories are created **only when that attempt actually starts** (not pre-created at run start).
